...
<footer class="bg-gray-900 text-white py-12">
  <div class="container mx-auto px-6 grid md:grid-cols-4 gap-8">
    <div>
      ...
      <p class="text-sm opacity-80">Dealer resmi Toyota terpercaya di Bandung dengan layanan purna jual terbaik.</p>
    </div>
    <div>
      <h4 class="font-semibold mb-4">Kontak</h4>
      <ul class="space-y-2 text-sm opacity-80">
        @if($contactEmail)<li><a href="mailto:{{ $contactEmail }}" class="hover:underline">{{ $contactEmail }}</a></li>@endif
        @if($contactPhone)<li><a href="tel:{{ $contactPhone }}" class="hover:underline">{{ $contactPhone }}</a></li>@endif
        @if($contactAddress)<li>{{ $contactAddress }}</li>@endif
      </ul>
    </div>
    <div>
      <h4 class="font-semibold mb-4">Layanan</h4>
      <ul class="space-y-2 text-sm opacity-80">
        <li><a href="#cars" class="hover:underline">Unit Baru</a></li>
        <li><a href="#cars" class="hover:underline">Unit Bekas</a></li>
        <li><a href="#promo" class="hover:underline">Promo</a></li>
        <li><a href="#test-drive" class="hover:underline">Test Drive</a></li>
      </ul>
    </div>
    <div>
  ...
  <div class="border-t border-gray-800 mt-8 pt-6 text-center text-xs opacity-60">
    &copy; {{ date('Y') }} {{ $companyName }}. Dealer resmi Toyota. All rights reserved.
  </div>
</footer>