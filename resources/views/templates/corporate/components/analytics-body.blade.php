...
<script>
    ...
    window.trackCustomEvent = function(eventName, eventData = {}) {
        ...
        if (window.customAnalyticsEndpoint) {
            ...
            }).catch(error => console.log('Custom analytics error:', error));
        }

        // Console log for debugging
        if ({{ app()->environment('local') ? 'true' : 'false' }}) {
            console.log('Custom Event Tracked:', eventName, { ...baseData, ...eventData });
        }
    };
    ...
    setInterval(function() {
        ...
        if (engagementTimer === 30) {
            trackCustomEvent('engagement_30s', { engagement_time: 30 });
        } else if (engagementTimer === 60) {
            trackCustomEvent('engagement_1min', { engagement_time: 60 });
        } else if (engagementTimer === 300) {
            trackCustomEvent('engagement_5min', { engagement_time: 300 });
        }
    }, 5000);
    ...
    window.addEventListener('scroll', function() {
        ...
        if (scrollPercent > maxScroll) {
            ...
            if (maxScroll >= 25 && maxScroll < 26) {
                trackCustomEvent('scroll_25', { scroll_depth: 25 });
            } else if (maxScroll >= 50 && maxScroll < 51) {
                trackCustomEvent('scroll_50', { scroll_depth: 50 });
            } else if (maxScroll >= 75 && maxScroll < 76) {
                trackCustomEvent('scroll_75', { scroll_depth: 75 });
            } else if (maxScroll >= 90 && maxScroll < 91) {
                trackCustomEvent('scroll_90', { scroll_depth: 90 });
            }
        }
    });
    ...
    document.addEventListener('visibilitychange', function() {
        if (document.hidden) {
            trackCustomEvent('page_hidden', { 
                engagement_time: engagementTimer,
