    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('cleanup_expired')
                ...
                ->color('danger')
                ->action(function () {
                    $expiredCount = WebsiteContent::where('expires_at', '<', now())
                        ->whereIn('status', ['draft', 'previewed'])
                        ->count();
                    
                    WebsiteContent::where('expires_at', '<', now())
                        ->whereIn('status', ['draft', 'previewed'])
                        ->delete();

                    Notification::make()
                        ->success()
                        ->title('Websites Cleaned Up')
                        ->body("Cleaned up {$expiredCount} expired draft websites")
                        ->send();
                })
                ->requiresConfirmation()
                ->modalHeading('Cleanup Expired Websites')
                ->modalDescription('This will permanently delete expired draft and previewed websites. Active websites will not be affected.')
                ->modalSubmitActionLabel('Delete Expired'),
